...
3. Enter any `Client ID` you want and click next. (e.g. aplicacion-demo)
4. Paste your new client id into the `.env` file next to `CLIENT_ID=`
5. Enable the following settings and click next:
...
8. Click save
9. From `Clients` go to the new client
10. Go to the `Credentials` tab
11. Copy the `Client Secret` and paste it in the `.env` file next to `CLIENT_SECRET=`

#### User Setup

1. Click on `Users` and `Add User`
2. Enter any `username` you want and click on `create` (e.g. cripto-user)
